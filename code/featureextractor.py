    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if buffer:
            ...
            if len(buffer)>=2:
                ...
                token2 = tokens[buffer_idx1]
                if FeatureExtractor._check_informative(token2['word'], True):
                    result.append('BUF_1_FORM_' + token2['word'])

                if FeatureExtractor._check_informative(token2['ctag']):
                    result.append('BUF_1_CTAG_' + token2['ctag'])

                if FeatureExtractor._check_informative(token2['tag']):
                    result.append('BUF_1_TAG_' + token2['tag'])
